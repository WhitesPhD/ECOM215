...
% \begin{frame}{The Settlement Chain}
%   \begin{center}
% \begin{tikzpicture}[
%     scale=1, every node/.style={transform shape},
%     node distance=0.5cm and 0.9cm,
%     box/.style={draw, rectangle, rounded corners, minimum width=1.3cm, minimum height=0.5cm, align=center, font=\tiny, fill=LightGray},
%     bluebox/.style={draw, rectangle, rounded corners, minimum width=1.3cm, minimum height=0.5cm, align=center, font=\tiny, fill=ImperialBlue!15},
%     arrow/.style={->, thin, >=stealth},
%     label/.style={font=\tiny, text=DarkGray}
%   ]
%     % Top row: traditional
%     \node[bluebox] (buyer) {Buyer};
%     \node[box, right=of buyer] (brokerB) {Buyer's\\Broker};
%     \node[box, right=of brokerB] (ccp) {Clearing\\House};
%     \node[box, right=of ccp] (brokerS) {Seller's\\Broker};
%     \node[bluebox, right=of brokerS] (seller) {Seller};
    
%     \node[box, below=0.6cm of ccp] (csd) {Depository};
    
%     \draw[arrow] (buyer) -- (brokerB);
%     \draw[arrow] (brokerB) -- (ccp);
%     \draw[arrow] (ccp) -- (brokerS);
%     \draw[arrow] (brokerS) -- (seller);
%     \draw[arrow] (ccp) -- (csd);
    
%     \node[above=0.2cm of ccp, font=\tiny\bfseries, text=ImperialBlue] {Traditional Settlement (T+1)};
    
%     % Bottom: blockchain
%     \node[bluebox, below=1.6cm of buyer] (buyerB) {Buyer};
%     \node[box, right=1.8cm of buyerB] (chain) {Shared\\Ledger};
%     \node[bluebox, right=1.8cm of chain] (sellerB) {Seller};
    
%     \draw[arrow] (buyerB) -- node[above, label] {Cash} (chain);
%     \draw[arrow] (chain) -- node[above, label] {Security} (sellerB);
%     \draw[arrow] (sellerB) -- node[below, label] {Security} (chain);
%     \draw[arrow] (chain) -- node[below, label] {Cash} (buyerB);
    
%     \node[above=0.2cm of chain, font=\tiny\bfseries, text=ImperialBlue] {Blockchain Settlement (near-instant)};
%   \end{tikzpicture}
%   \end{center}
  
%   \vspace{0.3em}
%   \textbf{Key difference}: In traditional settlement, multiple institutions maintain separate records and must reconcile them. In blockchain-based settlement, a single shared ledger records ownership, and the exchange of cash and securities can happen \textit{simultaneously}---this is called \highlight{atomic settlement} or \highlight{delivery-versus-payment (DvP)}.
% \end{frame}
...
\begin{frame}{What Blockchain Could Change}
  ...
  \begin{itemize}
    ...
    % \item \textbf{Reduced intermediation}: Fewer parties in the chain means lower cost and fewer points of failure.
  \end{itemize}
...
\begin{frame}{Canton Network and Fnality}
  ...
  % \vspace{0.3em}
  % Both represent the industry's attempt to get the benefits of shared ledgers without using public blockchains or cryptocurrencies.
\end{frame}
...
%      BlackRock BUIDL & Tokenized T-bills on Ethereum & Live (\$500m+) & Qualified purchasers only \\
  ...
  % \vspace{0.3em}
  % BlackRock BUIDL (covered in Week 6) is notable because it uses a \textit{public} blockchain (Ethereum) rather than a private one---a different bet on where institutional finance is heading.
\end{frame}
...
\begin{frame}{Cross-Border CBDC Experiments}
  ...
  % \vspace{0.3em}
  % \textbf{Fnality} (mentioned earlier): Bank of England-approved system using tokenized central bank reserves for wholesale settlement among 17 banks.
  
  % \vspace{0.3em}
  % % FIGURE PLACEHOLDER: Map or table of major wholesale CBDC experiments
  % % Source: BIS Innovation Hub, Atlantic Council CBDC Tracker
  % \textit{[Figure: Major wholesale CBDC experiments worldwide. Source: BIS Innovation Hub]}
\end{frame}
