...
\begin{enumerate}[leftmargin=*]
\item \textbf{Blockchain provides options, not guarantees.} The technology enables trustless transactions, but users can choose to reintroduce intermediaries. Most do.

\item \textbf{``Not your keys, not your coins'' reflects a real risk.} Mt.\ Gox, FTX, and many others demonstrate that custodial risk is not theoretical.

\item \textbf{There is a genuine trade-off between security and usability.} Self-custody is safer against exchange failure but harder to use and unforgiving of user error.

\item \textbf{Intermediaries re-emerge around decentralised systems.} Exchanges, custodians, and other service providers fill gaps that the base protocol doesn't address.

\item \textbf{Regulation changes the trust model but doesn't eliminate trust.} Oversight may reduce risk, but it reintroduces institutional dependencies.
\end{enumerate}

\section*{Connection to Lecture Material}

Link back to Week 1 lecture concepts:
\begin{itemize}[leftmargin=*]
\item The role of intermediaries in traditional finance (and why blockchain was designed to bypass them)
\item Private keys and digital signatures---what it means to ``own'' cryptocurrency
\item The difference between custodial and non-custodial arrangements
\item Blockchain's core properties (immutability, transparency) vs.\ how people actually use it
\item Public blockchains as ``trust minimisation''---but only if you use them as designed
\end{itemize}

\section*{Notes for TA}

\begin{itemize}[leftmargin=*]
\item This is the first tutorial, so students may be less familiar with discussion format. Be prepared to prompt more actively.
\item Some students may have strong ideological views about self-custody (``never trust exchanges'') or pragmatic views (``exchanges are fine if regulated''). Both perspectives have merit; encourage engagement with the trade-offs.
\item If students ask about the recovered Bitcoin (200,000 BTC): Yes, some was found in an old wallet in 2014, and creditor repayments began in 2024. The recovery process was extraordinarily slow due to legal complexity across jurisdictions.
\item The case deliberately avoids detailed technical discussion of transaction malleability. If students ask, explain that it was a bug in how Mt.\ Gox tracked transactions, not a flaw in Bitcoin itself. The point is the operational failure, not the technical detail.
\end{itemize}
