...
\begin{enumerate}[leftmargin=*]
\item \textbf{The Merge was a major achievement with real trade-offs.} 99.95\% energy reduction is significant, but PoS introduces different centralisation and security considerations.

\item \textbf{PoW vs PoS is a genuine debate, not a settled question.} Both mechanisms provide security through different means. The ``right'' answer depends on threat models and values.

\item \textbf{Centralisation pressure exists in both systems.} Mining pools and staking pools both concentrate power. ``Decentralisation'' is a spectrum, not a binary.

\item \textbf{Blockchains are social systems, not just technical ones.} The ability to execute the Merge shows that protocol rules can change through coordination. Governance matters.

\item \textbf{Censorship resistance is under pressure.} Regulatory compliance by validators is already happening. The long-term implications for Ethereum's neutrality are unclear.
\end{enumerate}

\section*{Connection to Lecture Material}

Link back to Week 2 lecture concepts:
\begin{itemize}[leftmargin=*]
\item Consensus mechanisms: PoW vs PoS comparison, now illustrated with a real transition
\item The blockchain trilemma: The Merge didn't improve scalability directly---that requires Layer 2
\item Economic incentives: Staking rewards ($\sim$3--5\% APR) vs mining economics
\item Centralisation concerns: Mining pools vs staking pools, Lido's dominance
\item Energy consumption: Concrete numbers on the before/after
\end{itemize}

\section*{Notes for TA}

\begin{itemize}[leftmargin=*]
\item Some students may be Bitcoin maximalists who view the Merge negatively. That's a legitimate perspective; ensure it gets fair treatment in discussion.
\item The OFAC/censorship discussion can become politically charged. Keep the focus on the technical and economic trade-offs rather than specific policy views.
\item If students ask about Ethereum's future (sharding, etc.), you can briefly mention that the Merge was step one of a multi-phase roadmap, but detailed coverage comes in later weeks.
\item The 32 ETH staking requirement ($\sim$\$50--100k) is a significant barrier. Liquid staking (Lido, Rocket Pool) addresses this but creates its own centralisation. This is a tension, not a solved problem.
\end{itemize}
