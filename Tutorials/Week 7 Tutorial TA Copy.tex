...
\section*{Synthesis Points (Final 6 minutes)}

\begin{enumerate}[leftmargin=*]
\item \textbf{There is no reliable Bitcoin valuation model.} Network value and stock-to-flow provide directional intuition but fail quantitatively. This is an honest assessment, not a weakness of our analysis.

\item \textbf{The absence of cash flows is the core problem.} Traditional finance values assets by discounting future cash flows. Most cryptoassets do not generate cash flows, which makes standard tools inapplicable. This doesn't mean the asset is worthless---it means our tools are limited.

\item \textbf{Scarcity is necessary but not sufficient for value.} Bitcoin's fixed supply matters, but supply tells you nothing without demand. A model that ignores demand (S2F) will eventually fail.

\item \textbf{In practice, institutional allocators use risk budgets, not price targets.} The ``how much can we afford to lose?'' approach is more defensible than any fundamental valuation for an asset this volatile and this poorly understood.

\item \textbf{Intellectual honesty matters.} An analyst who admits uncertainty is more valuable than one who presents false precision. The CIO asked for rigour---and rigour sometimes means saying ``we don't know.''
\end{enumerate}

\section*{Connection to Lecture Material}

Link back to Week 7 lecture concepts:
\begin{itemize}[leftmargin=*]
\item Valuation frameworks: network value, stock-to-flow, cost of production, tokenomics (all four from lecture)
\item Risk-return characteristics: Sharpe ratio limitations, sample dependence
\item The ``no cash flows'' problem as the fundamental challenge for crypto valuation
\item Fat tails and drawdown risk---relevant to Analyst C's risk-budget approach
\end{itemize}

\section*{Notes for TA}

\begin{itemize}[leftmargin=*]
\item This tutorial works best as a structured debate. Consider splitting the class into three groups, each assigned to defend one analyst's position, then have them challenge each other before converging on a recommendation.
\item Students may arrive with strong prior views on Bitcoin's value (either bullish or dismissive). The goal is not to change their view but to force them to justify it rigorously. Push back on \textit{every} position.
\item The data tables are simplified and illustrative. If students question specific numbers, acknowledge this and redirect to the methodological question: even with perfect data, would the model be reliable?
\item The Sharpe ratio table from Analyst C may generate debate about whether Bitcoin's Sharpe ratio (0.71) makes it an attractive investment. Refer back to the lecture: Sharpe ratios assume normal returns, and crypto returns are fat-tailed. A 77\% drawdown is not captured by standard deviation.
\end{itemize}
