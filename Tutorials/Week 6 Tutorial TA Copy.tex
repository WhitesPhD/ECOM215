...
% [FIGURE SUGGESTION: USDC price chart March 10-13, 2023, showing drop to ~$0.87]
...
\begin{enumerate}[leftmargin=*]
\item \textbf{Fiat-backed stablecoins inherit traditional banking risks.} USDC's problem wasn't its design---it was the failure of a bank where it held reserves. This is counterparty risk, not crypto risk.

\item \textbf{24/7 crypto markets clash with banking hours.} The weekend timing was critical. Without functioning redemptions, arbitrage couldn't restore the peg, and market prices reflected fear.

\item \textbf{Transparency creates its own dynamics.} Circle's disclosure was arguably correct, but it triggered the very panic it informed people about. Opacity (Tether) was paradoxically rewarded.

\item \textbf{Government safety nets matter.} The US government backstop resolved the crisis. Without it, USDC might have faced real solvency problems. Crypto is less ``independent'' of traditional finance than often claimed.

\item \textbf{``Safe'' is relative.} All stablecoins---fiat-backed, crypto-backed, algorithmic---have risks. The question is which risks you're exposed to and whether you understand them.
\end{enumerate}

\section*{Connection to Lecture Material}

Link back to Week 5 lecture concepts:
\begin{itemize}[leftmargin=*]
\item Fiat-backed stablecoin mechanics (mint/redeem arbitrage)
\item Reserve composition and quality
\item Bank run dynamics and the ``run risk'' of stablecoins
\item Comparison with Terra/Luna (different failure mode---counterparty vs.\ design)
\end{itemize}

\section*{Notes for TA}

\begin{itemize}[leftmargin=*]
\item This case is more recent and may be less familiar than Terra/Luna. Ensure students understand the SVB context before diving into USDC specifics.
\item The timeline matters: Friday disclosure $\rightarrow$ weekend with no redemptions $\rightarrow$ Sunday government announcement $\rightarrow$ Monday recovery. Walk through this if needed.
\item The government backstop question can get political. Keep focus on the economics and trade-offs.
\end{itemize}
