...
\begin{enumerate}[leftmargin=*]
\item \textbf{``Code is law'' is aspirational, not absolute.} Smart contracts execute code literally, but they exist within social, legal, and economic systems that involve human judgment.

\item \textbf{Immutability is a spectrum.} Even ``immutable'' blockchains can be forked by social consensus. The question is when such interventions are legitimate.

\item \textbf{Security is hard.} Known vulnerabilities weren't fixed. This remains a pattern---speed to market often wins over security.

\item \textbf{Responsibility is unclear.} Current systems place most risk on users least able to evaluate it. This is an unresolved tension with regulatory implications.

\item \textbf{History repeats.} The fundamental dynamics of The DAO (experimental code, concentrated funds, known bugs, delayed response) recur in modern DeFi exploits.
\end{enumerate}

\section*{Connection to Lecture Material}

Link back to Week 3 lecture concepts:
\begin{itemize}[leftmargin=*]
\item Smart contract properties (immutable, deterministic, atomic)
\item ``Code is law'' and its limitations
\item The DAO hack as the canonical smart contract failure
\item Upgradeability patterns that emerged in response
\end{itemize}

\section*{Notes for TA}

\begin{itemize}[leftmargin=*]
\item Students may have strong opinions about whether the fork was right. That's fine---encourage debate, but ensure both sides get air time.
\item If students ask about the attacker's identity, some later research (2022) claimed to identify them, but no legal action resulted. You don't need to go into detail; the case is about principles, not attribution.
\item The reentrancy bug is explained in the case brief at a conceptual level. You don't need to explain the code mechanics.
\end{itemize}
